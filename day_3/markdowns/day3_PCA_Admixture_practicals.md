...
![](https://github.com/nt246/physalia-lcwgs/blob/main/day_3/img/Silverside_Sample_Map.png?raw=true =100x)
...
In this tutorial, we want to use these data to determine the population structure of Atlantic silverside along its distribution range using principal components analysis (PCA)and admixture analysis. 
In addition, we will use principal components analysis and ancestry analyses to determine which populations most likely contain alternative inversion karyotypes.
...
Here we will estimate the covariance matrix using single-read sampling in [ANGSD](http://www.popgen.dk/angsd/index.php/PCA_MDS) as discussed during the lecture, but will also show you how to estimate the covariance matrix using[PCAngsd](http://www.popgen.dk/software/index.php/PCAngsd)


The first question is how our input dataset should look like? 


We only want to focus on variant sites in our population structure analyses. 
As we have learnt yesterday, we could just perform SNP calling in ANGSD and get a list of all variant sites in our dataset (WE ARE NOT RUNNING THIS CODE TODAY)
```
# angsd -b ALL_bams.txt -anc $REF -out Results/MME_SNPs \
#	-minMapQ 20 -minQ 20 -doMaf 1 -minMaf 0.05 -SNP_pval 2e-6 \
#	-GL 1 -doGlf 2 -doMajorMinor 1 -doPost 1
```

We could3 then extract a list of all variant sites from the minor allele frequency file:
```
...
In this practical, we will perform the population structure analysis using an LD-pruned SNP dataset that you prepared earlier today.
Later on we will use the full SNP dataset. The full SNP dataset will help us to understand how the inversion karyotype is distributed in our dataset and will highlight the impact of large LD clusters on the inferred structure. 
...
```
angsd -b $DIR/ALL_bams.txt -anc $REF -out Results/MME_ANGSD_PCA_LDpruned \
...
#Save plot as pdf
ggsave(filename = "~/day3/Results/pca_LDpruned_plot.pdf", plot = pca)
```
...
#Save plot as pdf
ggsave(filename = "~/day3/Results/pca_LDpruned_pcangsd_plot.pdf", plot = pca)
```
