...
In this tutorial, you will learn how to take the raw sequencing files in `fastq` format, and convert them into alignment files in `bam` format. Along the way, you will perform multiple quality control (QC) procedures, and will map the short sequences to a reference genome. 
...
* Copy the fasta file of the reference genome (`mme_physalia_testdata_chr24.fa`) and the Nextera adapter file (`NexteraPE_NT.fa`) to the `reference` folder
...
For this excercise, the sample list can be created using the following command. You may also use the `nano` command to edit text files in Unix shell.
...
For this excercise, the sample table can be created using the following command. 
...
You can run `cat /workdir/physalia-lcwgs/day_1/sample_lists/sample_table.tsv` to check the structure of the sample table.
